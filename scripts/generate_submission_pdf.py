...
def main():
    ...
    story += [
        ...
        Spacer(1, 12),
        p("SETU", styles["Title"]),
        p("Strategic Energy Trade Uncertainty", styles["Lead"]),
    ...
    story += [cover, Spacer(1, 25), p("Working prototype | React + FastAPI + SQLite | Source evidence + simulation + human-in-the-loop decisions", styles["Small"]), PageBreak()]
    ...
    story += [
        ...
        Spacer(1, 12),
        p("<b>Guided incident response:</b> one action runs a credible reference source through the complete sequence - source evidence -> HORMUZ risk update -> forecast -> cascade -> mitigation options. It provides a repeatable, inspectable operational workflow.", styles["Callout"]),
    ]
    ...
    story += [
        ...
        p("<font face=\"Courier\">git clone &lt;YOUR_GITHUB_REPOSITORY_URL&gt;<br/>cd SETU<br/>docker compose up --build</font>", styles["Body"]),
        p("Open <b>http://127.0.0.1:5173</b>, select <b>Run Incident Response Workflow</b>, then inspect the evidence and recommendation panels. The API health endpoint is available at <b>http://127.0.0.1:8000/health</b>.", styles["Body"]),
        Spacer(1, 12),
